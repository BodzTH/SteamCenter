...
{
  size_t bytesRead;
  // Use i2s_read instead of i2s_read_bytes
  esp_err_t result = i2s_read(I2S_NUM_0, (void *)data, numData, &bytesRead, portMAX_DELAY);
  return (result == ESP_OK) ? bytesRead : -1; // Return number of bytes read or -1 on error
}

void I2S_Write(char *data, int numData)
{
  size_t bytesWritten;
  // Use i2s_write instead of i2s_write_bytes
  esp_err_t result = i2s_write(I2S_NUM_0, (const void *)data, numData, &bytesWritten, portMAX_DELAY);
  // Optionally check result or bytesWritten for error handling
}
